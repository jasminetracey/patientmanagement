...
const ButtonClose = ({ color, onClick }) => {
    const className =
        color === "red"
            ? "block w-2 h-2 fill-current text-red-700 group-hover:text-red-800"
            : "block w-2 h-2 fill-current text-green-700 group-hover:text-green-800";
    ...
    return (
        <div>
            {flash.success && visible && (
                <div className="mb-8 flex items-center justify-between bg-green-500 rounded w-full">
            ...
            {(flash.error || numOfErrors > 0) && visible && (
                <div className="mb-8 flex items-center justify-between bg-red-500 rounded w-full">
                    <div className="flex items-center">
                        <IconDanger />
                        <div className="py-4 text-white text-sm font-medium">
